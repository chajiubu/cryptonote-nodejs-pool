...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'BlockTemplate':
            ...
            try {
                if (!currentBlockTemplate[0] || new_hash.toString('hex') !== currentBlockTemplate[0].prev_hash.toString('hex')) {
                    log('info', logSystem, 'New %s block to mine at height %d w/ difficulty of %d', [config.coin, message.block.height, message.block.difficulty]);
                    if (mergedMining) {
	    ...
	    let poolIndex = parseInt(message.poolIndex)
            try {
                if (!currentChildBlockTemplate[poolIndex] || message.block.height > currentChildBlockTemplate[poolIndex].height) {
                    log('info', logSystem, 'New %s child block to mine at height %d w/ difficulty of %d', [config.childPools[poolIndex].coin, message.block.height, message.block.difficulty]);
...
Miner.prototype = {
	...
	if (mergedMining) {
        ...
        if (this.lastBlockHeight === currentBlockTemplate[this.activeChildPool].height
        ...
        if (!this.proxy) {
            let blob = currentBlockTemplate[this.activeChildPool].nextBlob(this.activeChildPool);

            this.lastBlockHeight = currentBlockTemplate[this.activeChildPool].height;

            let target = this.getTargetHex();

            let newJob = {
                id: utils.uid(),
                extraNonce: currentBlockTemplate[this.activeChildPool].extraNonce,
                height: currentBlockTemplate[this.activeChildPool].height,
                childHeight: this.lastChildBlockHeight,
                difficulty: this.difficulty,
                diffHex: this.diffHex,
	            activeChildPool: this.activeChildPool,
                submissions: []
            };

            if (currentBlockTemplate[this.activeChildPool].isRandomX){
                newJob['seed_hash'] = currentBlockTemplate[this.activeChildPool].seed_hash
                newJob['next_seed_hash'] = currentBlockTemplate[this.activeChildPool].next_seed_hash
            }
            ...
            let blob = currentBlockTemplate[this.activeChildPool].nextBlobWithChildNonce();
            this.lastBlockHeight = currentBlockTemplate[this.activeChildPool].height;
            let target = this.getTargetHex();
            ...
            let newJob = {
                ...
                difficulty: this.difficulty,
                diffHex: this.diffHex,
                clientPoolLocation: currentBlockTemplate[this.activeChildPool].clientPoolLocation,
                clientNonceLocation: currentBlockTemplate[this.activeChildPool].clientNonceLocation,
	            activeChildPool: this.activeChildPool,
                submissions: []
            };
            ...
            this.cachedJob = {
                ...
                difficulty: currentBlockTemplate[this.activeChildPool].difficulty,
                height: currentBlockTemplate[this.activeChildPool].height,
                childHeight: this.lastChildBlockHeight,
                reserved_offset: currentBlockTemplate[this.activeChildPool].reserveOffset,
                client_nonce_offset: currentBlockTemplate[this.activeChildPool].clientNonceLocation,
                client_pool_offset: currentBlockTemplate[this.activeChildPool].clientPoolLocation,
        ...
        if (this.lastBlockHeight === currentBlockTemplate[this.activeChildPool].height && !this.pendingDifficulty && this.cachedJob !== null) {
            return this.cachedJob;
        }
        if (!this.proxy) {
            let blob = currentBlockTemplate[this.activeChildPool].nextBlob();
            this.lastBlockHeight = currentBlockTemplate[this.activeChildPool].height;
            let target = this.getTargetHex();
        ...
        if (typeof config.includeHeight !== "undefined" && config.includeHeight)
            this.cachedJob['height'] = currentBlockTemplate[this.activeChildPool].height      
        return this.cachedJob;
    },
